function [ephemeris, ALLTckResult, for_prest] = naviDecode_updated(Acquired, ALLTckResult)
%Purpose
...
startOffset = 3000; % avoid noise due to tracking loop transition, 28/04/2020, BING XU
...
for svindex = 1 : length(Acquired.sv)
    ...
    msdatalength                                = length(NaviDatams);
        ...
        % To avoid outliers due to noise, 28/04/2020, BING XU
    for i = 2:length(NaviDatams)-1
        if (sign(NaviDatams(i-1)) == sign(NaviDatams(i+1))) && ...
                (sign(NaviDatams(i)) ~= sign(NaviDatams(i-1)))
            NaviDatams(i) = -NaviDatams(i);
        end
    end
        ...
        if (msdatalength-index) > 100 % 100 because C/N0
            ...
            if mod(idx,20) == 0
                ...
                if tempx > 17
                    ...
                    NaviDataXOR(idx2)   = 0;%NaviDataXORms(index);
                    NaviData(idx2)      = 1;% NaviDatams(index);
                    tempx = 0;
                end
                if tempx < -17
                    %                 NaviDataXOR(idx2)   = NaviDataXORms(index);
                    ...
                    NaviDataXOR(idx2)   = 1;%NaviDataXORms(index);
                    NaviData(idx2)      = -1;% NaviDatams(index);
                    tempx = 0;
                end
