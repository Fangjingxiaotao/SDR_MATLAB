...
estPos      = navSolutionsCT.usrPos(file.skiptimeVT/(solu.navSolPeriod),:);%cnslxyz;%navSolutionsCT.usrPos(file.skiptimeVT/(solu.navSolPeriod),:);  
estVel      = navSolutionsCT.usrVel(file.skiptimeVT/(solu.navSolPeriod),:);%zeros(1,3);%navSolutionsCT.usrVel(file.skiptimeVT/(solu.navSolPeriod),:); 
...
for svindex = 1:svlength
    ...
    Code(svindex,:)         = [codetemp(end) repmat(codetemp,1,track.pdi) codetemp(1)];
...
for msIndex = 1:datalength/track.pdi % 
    ...
    for svindex = 1:length(Acquired.sv)
        ...
        indx = 1;
        this_ceil1= ceil(t_CodeEarly) + indx;  % for check
        this_ceil2= ceil(t_CodePrompt) + indx;
        this_ceil3= ceil(t_CodeLate) + indx;
        ceil_mx = [this_ceil1; this_ceil2; this_ceil3];
        
        for idx = 1:3
            if(max(ceil_mx(idx))>1025)
                for ind1 =1: length(ceil_mx(idx))
                    if ceil_mx(idx,ind1)>1025
                        ceil_mx(idx,ind1)=1025;
                    end
                end
            end
        end
...
%         if (max(ceil(t_CodeEarly) + indx)>1025)          
%             this_ceil;     
%             for ind1 =1: length(this_ceil1)
%                 if this_ceil1(ind1)>1025
%                     this_ceil1(ind1)=1025;
%                 end
%             end
%         end
        
        CodeEarly      = Code(svindex,(ceil_mx(1)));
        CodePrompt     = Code(svindex,(ceil_mx(2)));
        CodeLate       = Code(svindex,(ceil_mx(3)));
%         CodeLate       = Code(svindex,(ceil(t_CodeLate) + indx));
    ...
    for svindex = 1:svlength
        ...
        %prr_measured(svindex)	= (carrFreq(svindex) + signal.IF)*cmn.cSpeed/signal.Fc; %% 3 Sept 2021
