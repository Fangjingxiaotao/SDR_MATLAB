...
if file.dataPrecision == 2
    rawsignal = fread(file.fid,signal.Sample*file.dataType*acq.datalen,'int16')'; % 20ms data for acquisition
    ...
    rawsignal = fread(file.fid,signal.Sample*file.dataType*acq.datalen,'int8')'; % 20ms data for acquisition
    if file.dataType == 2
...
for svindex = 1:32
    ...
    if SNR >=12% 16  % acquisition thredhold
        ...
        fprintf(' SV[%2d] SNR = %2.2f, Code phase = %5d, Raw Doppler = %5d \n ', ...
            svindex,SNR,codePhase-1, Doppler);
    end
...
if isempty(Acquired.sv)
    ...
    fprintf('Now refining Doppler freq... \n ');
    fseek(file.fid,file.skip*signal.Sample*file.dataPrecision*file.dataType,'bof');
    if file.dataPrecision == 2
        ...
        if file.dataType == 2
            longrawsignal = longrawsignal(1:2:length(longrawsignal)) + 1i*longrawsignal(2:2:length(longrawsignal));
        end
    end
    ...
    for svindex = 1 : length(Acquired.sv)
        ...
        fftlength = length(CarrSignal) * acq.datalen;
        if file.dataType == 2
