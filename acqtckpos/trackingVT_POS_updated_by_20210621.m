...
clkBias     = navSolutionsCT.clkBias(file.skiptimeVT/(solu.navSolPeriod/2));   
clkDrift    = navSolutionsCT.clkDrift(file.skiptimeVT/(solu.navSolPeriod/2));
...
% parameters for NLOS detection and correction (IEEE TIM)
cnt_nlos = 0;
codeError_mean3 = 0;
codeError_mean22 = 0;
codeError_mean27 = 0; 
cnt_correction = 0;
codeError_20 = zeros(1,20);
...
sampleStartTckVT = measStartCT + file.skiptimeVT/(solu.navSolPeriod/2); % First sample for VT, same to all channels
...
for svindex = 1:svlength
    ...
    codeFreq(svindex)       = TckResultCT(Acquired.sv(svindex)).codeFreq(fix(sampleStartTckVT/2/signal.Fs));
    codePhaseStep(svindex)  = codeFreq(svindex)/fs;
    remChip(svindex)        = TckResultCT(Acquired.sv(svindex)).remChip(fix(sampleStartTckVT/2/signal.Fs));
    carrFreq(svindex)       = TckResultCT(Acquired.sv(svindex)).carrFreq(fix(sampleStartTckVT/2/signal.Fs));
    oldCarrFreq(svindex)    = TckResultCT(Acquired.sv(svindex)).carrFreq(fix(sampleStartTckVT/2/signal.Fs));
    remCarrPhase(svindex)   = TckResultCT(Acquired.sv(svindex)).remCarrPhase(fix(sampleStartTckVT/2/signal.Fs));
    file_ptr(svindex)       = TckResultCT(Acquired.sv(svindex)).absoluteSample(fix(sampleStartTckVT/2/signal.Fs));
    carrFreqBasis(svindex)  = TckResultCT(Acquired.sv(svindex)).carrFreq(fix(sampleStartTckVT/2/signal.Fs));
    codedelay_tck(svindex)  = TckResultCT(Acquired.sv(svindex)).codedelay(fix(sampleStartTckVT/2/signal.Fs));
    oldCarrError(svindex)   = TckResultCT(Acquired.sv(svindex)).carrError(fix(sampleStartTckVT/2/signal.Fs));
    oldCarrNco(svindex)     = TckResultCT(Acquired.sv(svindex)).carrFreq(fix(sampleStartTckVT/2/signal.Fs)) - carrFreqBasis(svindex);
...
for msIndex = 1:datalength/pdi % 
    ...
    %     [pseudorangeVT, relative_time] = pr_est(Acquired, sbf, signal,TckResultVT, msIndex);
    %     for svindex = 1:svlength
    %         prn = sv(svindex);
    %         TckResultVT(prn).rawPseudorange(msIndex) = pseudorangeVT(svindex)  + correction(svindex)*293;
    %         TckResultVT(prn).truePseudorange(msIndex) = norm(cnslxyz(1:3) - svxyz_tck(svindex,:));
    %     end
...
%         tot_est_pos(svindex) = tot_est_tck(svindex);      
        ...
        prr_measured(svindex)	= (carrFreq(svindex) - signal.IF)*cmn.cSpeed/fL1;%
    ...
    if flag_corrCovEst2 == 1 && counterUptR == thresUptR
        ...
        mesurement_noise(svlength+1:2*svlength,svlength+1:2*svlength) = ...
            tmpR(svlength+1:2*svlength,svlength+1:2*svlength)*1;% +((1/pdi )/track.PLLBW);
...
%     %% NLOS Correction
% %         	mesurement_noise(1,1) = 2e6;
%     %     	mesurement_noise(3,3) = 1e6;
%         	mesurement_noise(6,6) = 1e6;
%     
%             if msIndex >= 1000
%                cnt_nlos = cnt_nlos + 1;
%     
% %                codeError_mean3 = codeError_mean3 + codeError(1);
%                codeError_mean22 = codeError_mean22 + codeError(4);
%     %            codeError_mean27 = codeError_mean27 + codeError(5);
%     
%                 if cnt_nlos >= 20/pdi    %
%                     cnt_correction = cnt_correction + 1;
%                     
% %                     codeError_mean3 = codeError_mean3/(20/pdi);
% %                     if abs(codeError_mean3) > 0.02
% %                         mesurement_noise(1,1) = 50;
% %                         mesurement_noise(1+svlength,1+svlength) = 0.25;
% %                         correction(1) = codeError_mean3;
% %                     end
% %                     codeError_mean3 = 0;
%     
%     
%                     codeError_mean22 = codeError_mean22/(20/pdi);
%                     if abs(codeError_mean22) > 0.02
%                         mesurement_noise(4,4) = 50;
%                         mesurement_noise(4+svlength,4+svlength) = 0.25;
%                         correction(4) = codeError_mean22;
%                     end
%                     codeError_mean22 = 0;
%     %
%     % %                 codeError_mean27 = codeError_mean27/ (20/pdi);
%     % %                 if codeError_mean27 > 0.05
%     % %                     mesurement_noise(5,5) = 50;
%     % %                     mesurement_noise(5+svlength,5+svlength) = 0.25;
%     % %                     correction(5) = codeError_mean27;
%     % %                 end
%     % %                 codeError_mean27 = 0;
%     %
%                     cnt_nlos = 0;
...
% %         	mesurement_noise(1,1) = 2e6;
%     %     	mesurement_noise(3,3) = 1e6;
% %         	mesurement_noise(4,4) = 1e6;
%         
% codeError_20  = [codeError_20(2:end) codeError(4)];
% 
%             if msIndex >= 1000
% %                cnt_nlos = cnt_nlos + 1;
%      
% %                codeError_mean22 = codeError_20;% codeError_mean22 + codeError(4); 
%     
% %                 if cnt_nlos >= 20/pdi    %
% %                     cnt_correction = cnt_correction + 1;
%                      
%                     codeError_mean22 = sum(codeError_20)/(20/pdi);
%                     if abs(codeError_mean22) > 0.02
%                         mesurement_noise(4,4) = 50;
%                         mesurement_noise(4+svlength,4+svlength) = .25;
%                         correction(4) = codeError_mean22;
%                     end
% %                     codeError_mean22 = 0; 
%                     
...
%             record_correction(cnt_correction,:) = correction;
